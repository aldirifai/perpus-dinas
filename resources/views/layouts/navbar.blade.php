                ...
                @if (Auth::check())
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ request()->is('pinjam*') ? 'active' : '' }}"
                            href="{{ url('pinjam') }}">Peminjaman</a>
                    </li>

                    @if (Auth::user()->level != 3)
                        <li class="nav-item">
                            <a class="nav-link {{ request()->is('data-hilang*') ? 'active' : '' }}"
                                href="{{ url('data-hilang') }}">Data Hilang</a>
                        </li>


                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle {{ request()->is('report*') ? 'active' : '' }}"
                                href="#" id="report" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                Reporting
                            </a>
                            <ul class="dropdown-menu" aria-labelledby="report">
                                <li><a class="dropdown-item" href="{{ url('report/buku') }}">Laporan Buku</a></li>
                                <li>
                                    <a class="dropdown-item" href="{{ url('report/pinjam') }}">Laporan
                                        Peminjaman</a>
                                </li>
                                <li>
